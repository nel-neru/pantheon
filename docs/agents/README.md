...
Pantheon には、CLIから直接使うエージェントと、自己拡張/内部支援に使うエージェントの両方が存在します。
...
| --- | --- | --- | --- | --- |
| `CodeReviewAgent` | 対象リポジトリを分析して改善提案を生成 | `DEEP_RESEARCH`, `PERFORMANCE_ANALYSIS` | `pantheon analyze`, Web `/api/analyze` | `apply_skills_to_prompt()` を使う代表例 |
| `ImprovementExecutorAgent` | 承認済み提案をコードへ反映し、PR/ローカルブランチを作成 | `PROMPT_ENGINEERING`, `TOOL_INTEGRATION` | `pantheon approve`, `SelfImprovementLoop` | GitHub token がなければローカル適用 |
...
- `SelfImprovementLoop` が `.pantheon/improvements/` から提案を取得
