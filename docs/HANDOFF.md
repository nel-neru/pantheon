...
**B) GitHub 経由** — リモート `nel-neru/repocorp_ai` は**未作成**（§6）。作成・push 後に:

```bash
git clone <github-url> repocorp_ai && cd repocorp_ai
git checkout phase5-improvements-handoff
```
...
- **GitHub リモート `nel-neru/repocorp_ai` は現時点で存在しません**（`gh` 上に未作成）。そのため `git push` は失敗します。GitHub を使うなら先にリポジトリ作成が必要（`gh repo create` 等）。`gh` は `na-masaoka`（active）/`nel-neru` で認証済み・SSH。
- **当面の移行は git bundle を使用**: `~/Downloads/repocorp_ai-handoff.bundle`（全履歴入り）。§1-A の手順で別デバイスに clone。
