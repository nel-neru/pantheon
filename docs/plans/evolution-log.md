...
Cycle 63 — (正確性/高 stakes mutating) 自律 AUTO_APPROVE 改善適用のブランチ stacking バグを根治＝improvement_executor を適用後に元ブランチへ復帰させ各 improvement ブランチを独立化  (2026-06-20)
  Plan   : C62 が筆頭 Next（fff）に挙げた **最高 stakes の未監査 mutating 経路 `agents/improvement_executor_agent`**（承認済み提案を適用しブランチ/PR を作る）へ HIGH 確証 bug-hunt を投下。実コードを読み呼び出し側（core/scheduler）を辿って **実バグを1件確証**:
           `_apply_local_change` が `git checkout -b <improvement-branch>` → 書込/commit → **元ブランチへ戻さず return**。scheduler `_apply_proposal` は `for s in suggestions:` の **一括 AUTO_APPROVE ループ**で提案ごとにこれを呼ぶため、2件目以降の `checkout -b` が**直前の improvement ブランチを base に切られ branch が積み重なる**（branchN が proposal 1..N の全コミットを含む＝個別レビュー/PR 不能・自律改善デーモンの実害）＋作業ツリーが最後の improvement ブランチに置き去り。
           **なぜ今これか**: C62 の指針「易しい正確性の井戸は枯れ気味＝高 stakes mutating 経路へ網を上げる」に直撃・HIGH 確証の実バグ・production 経路（improvement daemon）・最小可逆（適用前ブランチを控えて復帰するだけ）。受け入れ基準=元ブランチ復帰・実 git でブランチ独立性を証明する load-bearing 回帰テスト・全 GREEN・敵対レビュー pass・merged。
           **落とした候補**: (ggg) vision/DX publishing=より大きいスライスで次サイクルへ、(hhh) core/quality=既に着手中の executor に1サイクル集中。
  Did    : work/improvement-executor-branch-restore-20260620（main 58d2e6f）。`_apply_local_change` で `checkout -b` の**前**に `original_ref = repo.active_branch.name`（detached HEAD は `TypeError`→`repo.head.commit.hexsha` にフォールバック）を控え、commit **後**に `repo.git.checkout(original_ref)` で復帰。
           復帰は **best-effort**（commit は成立済＝適用自体は成功なので失敗に倒さず warning のみ・汚れた作業ツリー等の別 failure class で握り潰さない）。回帰テスト2本追加: ①ダミー mock で `checkout_calls[-1] == ("main",)`（復帰を実証）、②**実 git**（GitPython）で2提案を連続適用→各 branch が自分の変更だけ含み base から1コミット（`a.py∈branchA・∉branchB` 等）＝stacking しないことを証明。
  Check  : ruff クリーン ／ **test-triage 全件 GREEN（1673 passed・+2・既知2失敗のみ・回帰0）**。**load-bearing 実証**: 復帰処理を一時無効化→新テスト2本とも fail（dummy は `checkout_calls` が `[("-b",b)]` のみ・実 git は branchB が A を含み `rev_list base..B==2`）後 restore。
           **敵対的レビュー code-reviewer = APPROVE（blocking 0）**: ① PR 経路（github_token 有）は `_apply_local_change` を通らず無影響を確認、② scheduler の per-proposal ループが実バグ源と確認、③ 実 git で「commit-on-branch→checkout base で新規ファイルは作業ツリーから除去・tree は元の clean 状態」を再現＝self-extension の新規ファイルが base に漏れない、④ detached-HEAD 判別（`active_branch` の TypeError）は GitPython の正しい仕様・SHA checkout は妥当な re-detach 復帰、⑤ best-effort swallow は `success=True` を不正直にしない（関数契約=「改善は自分の branch に commit 済」は復帰前に達成）。nit（復帰失敗時は次提案で stacking 再発しうる点を明記）を採用しコメント追記。
  Act    : merged ✅（merge_to_main ゲート通過・--delete-branch、リモート削除エラーは未 push で benign）＝production robustness（自律改善デーモンの一括適用で各 improvement ブランチが独立・作業ツリーが置き去りにならない）。固定化（memory [[autonomous-review-loop]] / [[work-branch-commit-discipline]] に C63 archetype 追記）:
           **archetype「checkout -b で作業ブランチを切る mutating ヘルパは、適用後に元ブランチへ best-effort 復帰せよ」**＝per-item ループで呼ばれる git mutating 関数が HEAD を移したまま return すると、次反復が前回のブランチを base に積まれ各成果物が独立しない（branch stacking）＋作業ツリーが置き去る。検証は**実 git の小リポを作り2件連続適用→各 branch が自分の変更だけ＋base から1コミット**を load-bearing 化（dummy mock では checkout no-op で stacking を再現不能＝実 git が必須）。**commit メッセージにバッククォートを含めると bash のコマンド置換が発火**＝`git commit -F <file>`（または PowerShell here-string）で渡す。
  Next   : C64 候補 — (iii) `agents/code_review_agent`（残るもう1つの未監査 agent・分析経路）or `core/quality` SelfImprovementLoop を HIGH bug-hunt、(ggg) **vision/DX スライス**（publishing `_publish_live` 承認ゲート前進 or atelier 未配線機能の最小配線）で correctness 連発から多様性転換、(jjj) `core/goals` の LLM 出力パース経路（C55 で execution は硬化済だが parse は別）。
           **C63 で高 stakes mutating 経路の1つを根治＝次は (a) 残る未監査 agent をもう1つ当てるか (b) cycle 種別を vision/DX へ転換（backend correctness が C55-63 で連続気味）。bug-hunt は HIGH のみ・実 git/実経路で検証・既掃討除外を継続。**

Cycle 64 — (正確性/堅牢性) 提案生成パイプラインの LLM 出力パースを lossy から防御的に＝余分なキー/null/非 dict で well-formed な提案を黙って落とさない  (2026-06-20)
  Plan   : C63 が改善パイプラインの**適用側**（mutating）を根治したので、C64 は同パイプラインの**生成側** `agents/code_review_agent`（不良/欠落提案→不良な自動適用に直結する高 stakes・未監査）へ HIGH bug-hunt。精読で **lossy な LLM 出力パースを1件確証**:
           `_generate_suggestions` が `CodeImprovementSuggestion(**raw)` を直接構築するため、**LLM が schema 外の余分なキー（severity/line/rationale 等＝頻出）を1つでも足すと TypeError で提案ごと丸ごと黙って drop**＝コア提案生成の収量が静かに減る。加えて `data.get("suggestions", [])` は **get-default-none 罠**（`"suggestions": null` で None を返し `for None` → TypeError → 再 raise → heuristic fallback）。
           **なぜ今これか**: 高 stakes・未監査 agent・HIGH 確証・[[get-default-none-footgun]] と R4「defensive deserialization」archetype に合致・最小可逆・実消費者は改善パイプライン全体。**vision/DX live 投稿前進は無人運転で安全に検証不能なので後回し**。受け入れ基準=余分キー許容・null/非 dict ガード・既知キーの検証は維持・全 GREEN・敵対レビュー pass・merged。**落とした候補**: (ggg) publishing live=無人で検証不能、(jjj) core/goals parse=次サイクルへ。
  Did    : work/code-review-suggestion-extra-keys-20260620（main faa8b3c）。`_SUGGESTION_FIELDS = {f.name for f in dataclasses.fields(CodeImprovementSuggestion)}`（import 時・drift-proof）を追加し、ループを ①`data.get("suggestions") or []`（null→空）②非 dict 要素を `isinstance` で明示スキップ ③`raw` を既知フィールド**かつ非 null** だけにフィルタしてから構築、へ変更。
           **検証は値に触れずキー集合だけ**＝既知キーの `__post_init__` 検証（priority/category/file_path の正規化・拒否）は不変・必須キー欠落は依然 drop・任意キーの明示 null は default に落ちる（None を下流 payload へ流さない＝nit 採用）。回帰テスト5本（余分キー許容/null suggestions/null 任意キー→default/必須欠落 drop/非 dict スキップ）。
  Check  : ruff クリーン ／ **test-triage 全件 GREEN（1676 passed・+3→最終 +5・既知2失敗のみ・回帰0）**。**load-bearing 実証**: 旧ループへ一時差し戻し→`tolerates_extra_keys`（提案 drop で空）と `handles_null_suggestions`（`for None` TypeError）が fail・`skips_non_dict` は旧でも pass（非 dict は旧 except でも skip＝documentation テスト）後 restore。
           **敵対的レビュー code-reviewer = APPROVE（blocking 0）**: ① フィルタは unknown キー除去のみで値に未介入＝`../escape.py`/`urgent` は依然 ValueError で drop、② 必須欠落は TypeError で drop（default 注入なし）、③ `_SUGGESTION_FIELDS` は dataclass 由来で drift-proof、④ 既存テスト不変、⑤ 新テスト load-bearing、⑥ `or []` は空 list/文字列/dict いずれも安全。nit2件（明示 null の default 上書き・必須欠落テスト）を**両方採用**して closeout。
  Act    : merged ✅（merge_to_main ゲート通過・--delete-branch、リモート削除エラーは未 push で benign）＝production robustness（提案生成の収量を LLM の schema ゆらぎから守る）。固定化（memory [[get-default-none-footgun]] に C64 として LLM-dataclass 版を追記）:
           **archetype「LLM 出力 dict → dataclass/Model 構築は『余分キー除去＋明示 null 除去』で防御的に」**＝`Model(**raw)` 直接は (a) 余分キー1個で TypeError→well-formed な項目ごと silent drop、(b) 明示 null が任意フィールドの default を None で上書き、の2系統で壊れる。正=`{k:v for k,v in raw.items() if k in fields(Model) names and v is not None}` でフィルタしてから構築（**値の検証は維持・キー集合だけ緩める**）。フィールド allowlist は `dataclasses.fields()` 由来にして drift-proof。`.get(k, default)` の None 罠と同根＝`.get(k) or default` / `or []`。
  Next   : C65 候補 — (kkk) **vision/DX スライスへ多様性転換**（backend correctness が C60除き C55-64 で連続＝種別を変える頃合い・atelier 未配線機能 or DX で安全＆vitest 検証可能なもの）、(jjj) `core/goals` LLM 出力パース経路を C64 archetype で横ぐし（goal_decomposer 等の `**raw`/`.get` 罠）、(lll) `core/quality` SelfImprovementLoop bug-hunt。
           **C63-64 で高 stakes agent 2つ（適用/生成）を根治＝correctness の旬は超えつつある。C65 は (a) vision/DX へ種別転換（無人で安全＆検証可能な範囲）か (b) C64 archetype の横ぐし（他の `**raw` LLM パース）を1本。bug-hunt は HIGH のみ・実経路で検証・既掃討除外を継続。**

Cycle 65 — (正確性/latent) goal_decomposer の Story description が title の複製になる copy-paste バグを根治（dead-path だが将来配線時の landmine）＝correctness の井戸枯れを確認し C66 を vision/DX へ転換  (2026-06-20)
  Plan   : C64 archetype（LLM dict パースの罠）の横ぐし(jjj)で `core/goals/goal_decomposer` を調査。**ただし memory の教訓「到達不能の修正は低価値→skip」を守り、fix と呼ぶ前に到達性を実コードで裏取り**。
           裏取り結果: `_decompose_with_llm` は `if use_llm and self._llm:` ゲート＋唯一の production 構築 `abstract_goal_pipeline:86` が `GoalDecomposer()`（llm_client 無し＝`self._llm=None`）＝**production では dead path**（テストのみが `_FakeLLM` 注入で行使）。調査中に **行682に実在の copy-paste バグ**を発見: `StorySpec.description=story_data.get("title","")`（Epic 676/Task 698 は正しく `"description"` 参照）＝Story の説明が title の複製になり本来の description が落ちる。
           **なぜ ship したか（dead-path でも）**: (a) 既に発見済みの1行修正で再調査コスト無し、(b) 既存テストハーネスあり、(c) **LLM 分解はロードマップ機能＝将来配線時に silent データ破損する landmine の除去**、(d) 値の検証でなくフィールド mapping の明白な誤り。**逆に「丸ごと1サイクルを dead-code hunt に費やす」のは避け**、最小 incidental 修正に留めた。受け入れ基準=1行 mapping 修正＋既存テストを Story description 検証で load-bearing 化・全 GREEN・敵対レビュー pass・merged。**落とした候補**: (kkk) vision/DX=C66 へ本格転換、(lll) core/quality=correctness 連発回避。
  Did    : work/goal-decomposer-story-description-20260620（main c456f4a）。`goal_decomposer._decompose_with_llm` の `description=story_data.get("title","")`→`story_data.get("description","")`（why コメント付き）。`tests/test_json_extract.py::test_goal_decomposer_llm_path_parses_nested_plan` の story JSON に `"description":"story desc"` を追加し `story.title=="Story A"`＆`story.description=="story desc"`（title と別値）を assert。
  Check  : ruff クリーン ／ **test-triage 全件 GREEN（1678 passed・既知2失敗のみ・回帰0）**。load-bearing: 旧コードは `story.description="Story A"`（title 複製）で新 assert と不一致＝fail（title/description を別値にして tautology 回避）。
           **敵対的レビュー code-reviewer = APPROVE（blocking 0）**: ① 修正は Epic(676)/Task(698) の sibling mapping と一致・他フィールド誤配線なし、② テスト load-bearing（旧コードで fail）、③ `StorySpec.description` の consumer は `to_dict()` とテストのみ＝buggy 挙動依存の consumer なし、④ dead-in-production 評価を確認（`self._llm=None` で経路到達不能・runtime 挙動変化ゼロ）。
  Act    : merged ✅（merge_to_main ゲート通過・--delete-branch、リモート削除エラーは未 push で benign）＝latent correctness hygiene（将来 LLM 分解配線時のデータ破損 landmine 除去・runtime 無変化）。固定化（本ログに井戸枯れシグナルを記録）:
           **correctness の井戸枯れ確認**＝C63-64 で高 stakes agent 2つを根治した後、C65 は **dead-path の latent バグしか見つからなかった**＝C62 が予告した「易しい正確性の旬は超えた」の決定的シグナル。**学び: 横ぐし調査で見つかるのが dead-path latent バグだけになったら、その層の cold-hunt は打ち切り、cycle 種別を correctness→vision/DX へ転換せよ**（dead-path の1行修正は ship してよいが、それを「高価値サイクル」と誤認しない＝正直さ）。memory [[get-default-none-footgun]] の「dead/live を分けて評価」を実践。
  Next   : C66 候補（**種別転換を強制**）— (kkk) **vision/DX スライス**を1本: 無人で安全＆検証可能な範囲＝(a) atelier GUI の未配線/未描画機能の最小配線（vitest で load-bearing 化）、(b) DX 改善（評価指標の可視化・開発ツール）、(c) publishing は live 投稿に触れない範囲のみ（preview/承認ゲートの UI/堅牢化）。
           **correctness backend は C55-65（C60 frontend 除く）で十分に枯れた＝C66 は必ず vision/DX or frontend へ転換し多様性を回復する。bug-hunt に戻るのは新しい高 stakes 未監査モジュール（core/quality SelfImprovementLoop 等）が出てきた時のみ。安全＆可逆＆検証可能を維持。**

Cycle 66 — (vision/DX・frontend) Marketplace に未開拓ジャンルの「起票しないプレビュー」を配線＝mutating scan しかなかった経路に look-before-leap を追加（build-but-unsurfaced 機能の発掘）  (2026-06-20)
  Plan   : C65 が強制した**種別転換（correctness→vision/DX）**(kkk) を実行。**重要な訂正**: C62-65 の Next が挙げた「atelier GUI」候補は memory [[atelier-gui]]/コミット 965bb0e で**撤去済み**＝正規フロントは `web/frontend` 一本（C66 開始時に再確認）。
           backend エンドポイントと frontend 消費の差分を洗い、**「非破壊プレビュー GET が backend にあるのに frontend は mutating scan POST しか露出していない」surfacing ギャップを2件**発見: ①`GET /api/hq/untapped-genres`（MarketplacePage・起票しないプレビュー）②`GET /api/hq/portfolio/plan`（RevenuePage・同型）。1サイクル=1焦点で **①** を選定（payload が richer＝genre/count/max_score/top_title でプレビュー表が意味を持つ・scan ボタンが同カードに既存）。
           **なぜ今これか**: C66 指針（vision/DX・無人で安全・vitest 検証可能・publishing live 非接触）に直撃。backend が専用の「起票しない プレビュー」を持つ＝surfacing 意図が明白なのに未配線。最小・可逆・frontend-only。受け入れ基準=プレビューボタン＋表描画＋空状態＋防御的 coerce＋load-bearing vitest・build/test 緑・敵対レビュー pass・merged。**落とした候補**: ②portfolio/plan プレビュー（同型→C67）、backend bug-hunt（井戸枯れ・多様性維持で回避）。
  Did    : work/untapped-genres-preview-20260620（main 8ae132e）。`MarketplacePage.tsx` に「未開拓ジャンルを確認」ボタン（Eye アイコン）を scan ボタン群へ追加し、`previewUntapped` callback で `GET /api/hq/untapped-genres` を呼んで結果を表（ジャンル/証拠数/最高スコア/代表トレンド）に描画。
           **payload は free-form なので防御的 coerce**（`Array.isArray` ガード・`Number.isFinite(Number(x))?:0`・`String(x??'')`＝[[get-default-none-footgun]] frontend 版）。state は `untapped: UntappedGenre[]|null`（null=未取得・[]=0件）＋`previewing`。`max_score.toFixed(1)` は coerce 後で常に有限。Tailwind は確立トークンのみ（`rounded-xl border border-white/10`＝当初の未定義 `bg-bg2/border-bd2` を修正）。vitest 2本追加。
  Check  : **frontend `npm run build`（tsc strict + vite）緑 ／ `npm test` 32 files・401 passed（+2）** ／ backend 無影響（frontend-only・merge_to_main の backend ゲートも通過＝既知2失敗のみ・回帰0）。**load-bearing 実証**: 描画ブロックを `{false && …}` で一時無効化→新テスト2本が fail（ジャンル名 `ai_devtools`・空状態メッセージとも未描画）後 restore。
           **敵対的レビュー code-reviewer = APPROVE（blocking 0）**: ① preview は厳密に read-only（GET 1本のみ・scan を誤発火しない・backend が「起票しない」と明記）、② `useCallback` deps `[]` 正（setter/api/toast のみ・stale closure 無し）、③ 防御 coerce は null/欠落/非配列 items・null 要素・null フィールドに堅牢で `toFixed` は never throw、④ Tailwind 全トークン定義済み（未定義トークン無し）、⑤ テストは load-bearing 非 tautological（GET 呼ぶ＆scan POST 呼ばない＆描画を assert・空状態も）、⑥ 他カード/scan/ConfirmDialog 無影響。nit2件（preview 中に scan を止めない disabled 非対称＝read-only なので妥当・`key={g.genre}` は backend が dedup 済）は**意図的に見送り**（defect でない）。
  Act    : merged ✅（merge_to_main ゲート通過・--delete-branch、リモート削除エラーは未 push で benign）＝vision/DX（mutating scan しかなかった HQ ジャンル発見に look-before-leap のプレビューを追加）。固定化（memory [[surfacing-promised-metrics]] に C66 として姉妹 archetype 追記）:
           **archetype「mutating-action-only：backend に非破壊プレビュー GET（起票しない/dry-run）の対があるのに frontend は mutating な scan/apply POST しか露出していない」**＝ユーザーが結果を見ずに盲目的に副作用を起こす UX defect。検出は **backend ルート列挙 vs frontend の `/api/...` 参照を差分**し「GET preview が消費されていない」対を探す（surfacing-promised-metrics の lede 起点とは別の入口＝**ペアになった非破壊 GET の存在**が surfacing 意図の証拠）。配線は frontend-only・read-only で最小可逆・vitest で「GET 呼ぶ＆mutating POST 呼ばない＆描画」を load-bearing 化。**学び: backend が dedicated な preview/dry-run エンドポイントを持つ＝それは surface されるべく作られた＝未配線なら高確度の vision/DX 候補**。同型の残候補=`GET /api/hq/portfolio/plan`（RevenuePage）。
  Next   : C67 候補 — (lll) **同 archetype の横ぐし**: `GET /api/hq/portfolio/plan` プレビューを RevenuePage に配線（scan で起票する前に target→gap→打ち手のプレビュー・C66 の直接横展開・frontend）、(mmm) backend ルート全体 vs frontend 消費の差分監査を完遂し他の mutating-only/未消費 preview GET を棚卸し（surfacing sweep の網を確定）、(nnn) DX 改善（評価指標の可視化・開発ツール）か core/quality SelfImprovementLoop bug-hunt（新規高 stakes 未監査なら correctness 復帰可）。
           **C66 で frontend surfacing archetype の新型を確立＝C67 は (a) 同型横ぐし（portfolio/plan）で複利化するか (b) surfacing sweep を完遂して棚卸しするのが高レバレッジ。種別は vision/DX/frontend を継続（correctness 復帰は新規高 stakes 未監査が出た時のみ）。安全＆可逆＆検証可能・nit は defect の時だけ直すを継続。**

Cycle 67 — (vision/DX・frontend) RevenuePage の自律経営プランに「起票しないプレビュー」を配線＝C66 mutating-action-only archetype の直接横展開（複利化）  (2026-06-20)
  Plan   : C66 が確立した **mutating-action-only archetype** の筆頭横ぐし候補(lll) を実行。RevenuePage「自律経営プラン（月収益目標）」カードは `POST /api/hq/portfolio/scan`（起票）しか無く、backend の `GET /api/hq/portfolio/plan?target=`（**起票しないプレビュー**: gap＋plan を返す）が未配線＝目標とのギャップも打ち手も見ずに盲目的に起票する UX。
           **なぜ今これか**: C66 と同型で確証高・1サイクルで複利化・frontend-only・read-only で最小可逆・vitest 検証可能。surfacing 意図は backend の dedicated preview エンドポイントの存在で明白。受け入れ基準=プレビューボタン＋gap/plan 描画＋防御 coerce＋load-bearing vitest・build/test 緑・敵対レビュー pass・merged。**落とした候補**: (mmm) surfacing sweep 完遂＝C68 へ、(nnn) DX/bug-hunt＝多様性で後回し。
  Did    : work/portfolio-plan-preview-20260620（main へ統合）。`RevenuePage.tsx` に「プランをプレビュー」ボタン（Eye・secondary）を「プランを起票」の隣に追加し、`previewPlan` callback で `GET /api/hq/portfolio/plan?target=${encodeURIComponent(...)}` を呼ぶ。
           gap サマリ（現状/目標/ギャップ/翌月予測＝`formatYen`・under_target で黄/緑）＋ plan エントリ（既存 `PortfolioProposal` 型・`kindLabel`/`kindBadge` を再利用）を描画。型 `PlanGap`/`PlanPreview` 追加。**防御的 coerce**（`Number.isFinite(Number(x))?:0`・`Boolean`・`Array.isArray` ガード・`String(x??'')`＝[[get-default-none-footgun]] frontend 版）。target 検証は `runTargetPlan` と同一。vitest 2本（GET 呼ぶ＆scan POST 呼ばない＆gap/plan 描画／プレビュー前は非描画）。
  Check  : **frontend `npm run build`（tsc strict）緑 ／ `npm test` 32 files・403 passed（+2）** ／ backend 無影響（frontend-only・merge_to_main の backend ゲート通過＝既知2失敗のみ・回帰0）。**load-bearing 実証**: 描画ブロックを `{false && …}` で一時無効化→プレビューテストが fail（`プランプレビュー（未起票）` 未描画）後 restore。
           **敵対的レビュー code-reviewer = APPROVE（blocking 0）**: ① 厳密に read-only（GET のみ・body 無し・scan POST を誤発火しない・test が `not.toHaveBeenCalledWith` で固定）、② query param `target` は backend 必須と一致・検証は scan と同一、③ `useCallback` deps `[targetInput]` 正・既存フロー無影響、④ 防御 coerce は null/欠落/非配列/null フィールドに堅牢・`formatYen` は負数（over-target の present_gap）も安全、⑤ Tailwind 全トークン定義済み（同ファイルで既出）、⑥ テスト非 tautological・`¥98,000`/plan title は他カードと非衝突（plan mock title を `[プラン]…` に変えて portfolio card の `[HQ提案]…` と区別）、⑦ key index-suffix で重複なし・既存挙動無影響。nit2件（`forecast_gap` coerce のみで未描画＝型が backend 契約を反映するため保持・翌月予測の重複表示＝gap 文脈で有用・Eye アイコン重複＝視覚のみ）は**defect でないため見送り**。
  Act    : merged ✅（merge_to_main ゲート通過・--delete-branch、リモート削除エラーは未 push で benign）＝vision/DX（目標→ギャップ→打ち手を起票前にプレビュー）。固定化（archetype は C66 で memory [[surfacing-promised-metrics]] に確立済＝C67 は**複利化の実証**としてログのみ）:
           **学び: archetype を確立した次サイクルは「同型横ぐし」が最小コスト最大確度＝C66 で書いた検出/配線/検証の型をそのまま再適用でき、レビューも同じ観点で素早く closeout できた（複利の実体）。** mutating-action-only の残候補は backend ルート全体 vs frontend 消費の差分監査（C68 mmm）で棚卸しすると網が確定する。
  Next   : C68 候補 — (mmm) **surfacing sweep 完遂**: `grep "@app.get" web/server.py` ↔ frontend `/api/...` 参照を全差分し、未消費の preview/dry-run GET（mutating POST と対のもの）を棚卸し→残があれば1本配線・無ければ「掃討済」を記録（網の確定）、(nnn) DX 改善（評価指標の可視化・開発ツール）で種別に変化、(ooo) core/quality SelfImprovementLoop bug-hunt（新規高 stakes 未監査なら correctness 復帰可）。
           **C66-67 で frontend surfacing archetype を確立＆1件複利化＝C68 は (a) sweep 完遂で網を閉じる（同種の打ち切り判断）か (b) DX/別種へ多様性転換。3連続 frontend surfacing は飽和シグナル＝C68 で sweep を閉じたら種別を変える。安全＆可逆＆検証可能・nit は defect の時だけを継続。**

Cycle 68 — (vision/DX・frontend) surfacing sweep を実行し最後の mutating-action-only ペアを配線＝不可逆な repo→workspace 移行の前に移行プラン（from_repo→to_workspace）を確認用に surface  (2026-06-20)
  Plan   : C67 が予告した (mmm) **surfacing sweep**＝backend 全 GET（`@app.get` 抽出）vs frontend `/api/...` 参照を全差分。未消費 GET を分類: ①**mutating POST と対の preview/dry-run GET**＝`GET /api/organizations/{name}/migration-plan`（対 `POST .../migrate-to-workspace`・OrgsPage が POST のみ使用）の **1件のみ**＝archetype 直撃。②その他の未消費 GET（businesses/memory-playbook/observability/workspace-db/chat-sessions）は **mutating POST と対でない standalone**＝弱い「backend が返すが未描画」型で archetype 対象外（観測専用 or 別機能・低レバレッジ）。
           ①を選定: **不可逆操作（"元に戻すことはできません"）なのに、ユーザーが具体的な移行先（from_repo→to_workspace）を見ずに確認している**＝最も価値の高い surfacing。受け入れ基準=確認ダイアログを開くと同時に副作用なし GET で具体パスを表示・POST は confirm 時のみ・既存 migrate テストの mock 更新＋専用テスト・build/test 緑・敵対レビュー pass・merged。**なぜ今これか**: archetype の最後のペア＆不可逆操作で surfacing 価値が最大・frontend-only・read-only で最小可逆。**落とした候補**: ②standalone 未消費 GET（archetype でなく低レバレッジ＝sweep 記録で closeout）、(nnn) DX/(ooo) bug-hunt＝種別転換は C69 へ。
  Did    : work/migration-plan-preview-20260620（main へ統合）。`OrgsPage.tsx` DetailPanel に `MigrationPlan` 型・`migrationPlan/planLoading` state・`openMigrateConfirm`（ダイアログを開くと同時に `GET .../migration-plan` を取得）を追加し、migrate ボタンの onClick を差し替え。ConfirmDialog の description に「移行元（repo）→ 移行先（workspace）」を描画（防御 coerce: `from_repo==null?null:String`・`String(to_workspace??'')`・`Boolean`、表示は `|| '—'`）。
           **GET 失敗でもダイアログは開く**（`setConfirmMigrate(true)` を try の前に置き、catch は toast のみ＝プレビュー欠落でも汎用警告で確認は可能）。`setMigrationPlan(null)` を開く度に実行し stale plan を防止。テスト: 既存 migrate テストの throw-on-unexpected mock に migration-plan GET を追加（さもないと破綻）＋専用テスト1本（GET 呼ぶ＆パス2件描画＆confirm 前は POST 呼ばない）。
  Check  : **frontend `npm run build`（tsc strict）緑 ／ `npm test` 32 files・404 passed（+1）** ／ backend 無影響（frontend-only・merge_to_main の backend ゲート通過＝既知2失敗のみ・回帰0）。**load-bearing 実証**: プラン描画ブロックを `{false && …}` で一時無効化→専用テストが fail（パス未描画）後 restore。
           **敵対的レビュー code-reviewer = APPROVE（blocking 0・nit1 を採用修正）**: ① 厳密に read-only（GET のみ・POST は confirm 時のみ・backend の plan は side-effect-free を確認）、② GET 失敗でもダイアログ開く（catch は toast）、③ modal は閉じないと別 org を開けない＝cross-org 遅延レスポンス到達不能で abort 不要・stale reset 正、④ 防御 coerce 堅牢・`|| '—'` は null/空とも正、⑤ テスト load-bearing 非衝突（`/repos/acme` は exact match で `/Users/test/repos/acme` と非衝突）、⑥ 既存 migrate テストの mock 更新は正当で POST+badge を依然 assert。**nit1=確定実バグ（Radix `Dialog.Description`=`<p>` に `<div>` 入れ子＝invalid DOM nesting）→ `<span className="block">` に修正（MarketplacePage 先例に整合）して closeout**。nit2（`already_workspace` 未使用）は backend 契約を反映する型として保持（migrate ボタンは repo モードのみ表示＝実質常に false・defect でない）。
  Act    : merged ✅（merge_to_main ゲート通過・--delete-branch、リモート削除エラーは未 push で benign）＝vision/DX（不可逆操作の前に具体的な移行先を確認＝最も価値の高い look-before-leap）。固定化（archetype は memory [[surfacing-promised-metrics]] に確立済＝C68 は sweep 完遂と打ち切り判断をログに記録）:
           **surfacing sweep 完遂＝mutating-action-only archetype は frontend で掃討済**（untapped-genres C66 / portfolio-plan C67 / migration-plan C68 の3ペアを配線）。残る未消費 GET は **standalone（mutating POST と対でない）**＝businesses/memory-playbook/observability/workspace-db/chat-sessions で、archetype 対象外（観測専用 or 別機能・低レバレッジ）。**学び: archetype の sweep は「全 site を機械的に差分し、対象（強い archetype）と非対象（弱い surfacing）を分類して網を閉じる」＝[[ruff-bug-scan-triage]] の三分規律の surfacing 版。3件で井戸が枯れたので C69 は種別転換（DX or backend bug-hunt）。**
  Next   : C69 候補（**種別転換**） — (nnn) DX 改善（評価指標の可視化・開発ツール・`.claude` ベストプラクティス採用）、(ooo) core/quality SelfImprovementLoop or 他の未監査 backend モジュールで HIGH bug-hunt（新規高 stakes なら correctness 復帰）、(ppp) standalone 未消費 GET のうち高価値なもの（observability summary 等）を1つ surface するか trend-watcher で `.claude` 更新提案を取り込む。
           **C66-68 で frontend surfacing archetype を3件で掃討完了＝C69 は必ず種別転換（frontend surfacing は飽和）。DX・backend bug-hunt・運用層硬化・`.claude` ベストプラクティスのいずれかへ多様性回復。安全＆可逆＆検証可能・nit は defect の時だけ直すを継続。**

Cycle 69 — (backend/正確性) 自己改善ループの sort-key が null `expected_impact` で TypeError → try/except に飲まれ「静かな永久停止」化するバグを根治＝C66-68 frontend surfacing から backend correctness へ種別転換  (2026-06-20)
  Plan   : C68 が必須とした **種別転換**を実行。core の確立済み archetype 残存を機械 grep（可変デフォルト引数=0・`datetime.utcnow`=0・os.kill/fcntl は C44/C42 で硬化済）で枯渇確認後、未監査の高 stakes 領域（`core/quality` 自己改善ループ＋`core/goals`）に bug-hunt subagent を投入。**確定バグ**: `SelfImprovementLoop.pickup_and_prioritize_proposals` の `sorted(raw, key=lambda p: (p.get("priority")=="high", p.get("expected_impact","")))`＝`get_pending_improvement_proposals` は **Pydantic 非経由で disk から生 dict** を返す（legacy/手編集/外部生成で `"expected_impact": null`＝None）→ 第1キー等値の2件比較で `None < str` の **TypeError**。これが scheduler `_maybe_run_self_improvement` の try/except に包まれ**可視 crash にならず自己改善が永久停止**（[[silent-drop-observability]] 同型・むしろ可視 crash より悪い）。**なぜ今これか**: 確立済み [[get-default-none-footgun]] archetype の伝播漏れ＝高確信・最小1行・本番経路2つ（scheduler 24/7＋CLI）で稼働・回帰検証容易。受け入れ基準=`expected_impact` 全 call site 一掃＋load-bearing 回帰＋既知2失敗のみ＋敵対レビュー pass＋merged。**落とした候補**: (nnn) DX/trend-watcher＝出力不確実で後回し、(ppp) standalone GET surfacing＝frontend で飽和、id フィールド等の別 get-default サイト＝非クラッシュ・theme 外で次サイクルへ。
  Did    : work/expected-impact-null-guard-20260620（main 4aeaffe へ統合）。`sorted(key=...)` の全 sort-key サイトを grep し **同一 `expected_impact` 式が loop:67＋graph:75 の2ファイル**＋graph:84 の `current.get("expected_impact","").lower()`（null で `None.lower()` AttributeError）＝**2ファイル3式**を `(... or "")` で coerce。敵対レビューが検出した web consumer `api_list_business_proposals`（`web/server.py` 同フィールドを同じ生 dict から payload へ）も型契約 str に揃えて coerce＝`expected_impact` footgun を完全閉鎖。回帰テスト3本（loop=実 disk JSON を `improvements/` に直書きして本番経路を忠実再現／graph=sort 比較＋approval `.lower()` 分岐）。
  Check  : **test-triage GREEN（backend 1680 passed・既知2失敗=chmod 0o600 のみ・新規回帰0）／ ruff check+format クリーン ／ web テスト 139 passed（同2失敗のみ）**。**load-bearing 実証**: 3式を旧コードへ一時差し戻し→新規3テストが期待どおり fail（TypeError×2／`AttributeError: 'NoneType' object has no attribute 'lower'`×1）後 restore。**敵対的レビュー code-reviewer = APPROVE（blocking 0）**: ① 旧 crash・新 non-crash・非 null 時 **bit-for-bit 等価**（ソート順・human_approval_required 不変）を実証、② テストは非 tautological・loop は実 disk read 経路で faithful、③ `expected_impact` 全サイト網羅、④ 🟡 web/server.py:2681 を同型 twin として検出→**本サイクルに畳み込み closeout**（benign-rendering だが footgun-completeness のため）、⑤ 規約遵守（404 不変・新 datetime 無し）。
  Act    : merged ✅（merge_to_main ゲート通過・--delete-branch、リモート削除エラーは未 push で benign）＝backend correctness。固定化: memory [[get-default-none-footgun]] に **第3系統「sort-key/order-comparison 版」** を追記＝`.get(k,"")` を比較/`.lower()`/`in` 位置で使うと crash、しかも try/except 包囲の load-bearing loop だと「静かな永久停止」に化ける（可視 crash より悪い）。canonical 形は `x.get(k) or default` と `str(x.get(k))` の2通り（manager.py:168 は後者で既に防御済）。**学び: 確立 archetype の「伝播漏れ」は新規バグ探索より高 ROI＝grep で全 call site を機械的に当て、単一フィールドの theme に絞って一掃しつつ兄弟フィールドは次 sweep へ明示記録。**
  Next   : C70 候補 — (qqq) **sort-key null coerce sweep**（C69 grep が surface した兄弟サイト: `task_queue.py:274` の `int(t.get("priority",5))`=null で `int(None)` TypeError＋`created_at`／`task_queue.py:220`／`core/knowledge/manager.py:232`／`core/orchestration/task_matcher.py:45`／`web/server.py:1145`＝**生 dict sort-key を一律 `str(...)/or default` 化**・task daemon は 24/7 drain なので priority/created_at は live）、(rrr) DX/種別転換（trend-watcher で `.claude` 更新 or 評価指標可視化）、(sss) `id` 等の別 get-default-none サイト（self_improvement_loop:128/131/134 の `str(prop.get("id",""))`=null で fabricated "None" id）。
           **C69 で backend correctness 復帰＝C70 は (qqq) で同 archetype の兄弟 sweep を1本で閉じる（task_queue の `int(None)` は実クラッシュで優先）か、種別転換で DX/運用層へ。3連続 correctness を避け多様性を保つ。安全＆可逆＆検証可能を継続。**

Cycle 70 — (backend/正確性・sweep) C69 が surface した兄弟 sort-key null footgun を archetype 全体で掃討＝task_queue/knowledge/web の生 dict sort-key を coerce し「数値キーは `0 or default` で 0 を破壊する」逆方向の罠も根治  (2026-06-20)
  Plan   : C69 予告の (qqq) **sort-key null coerce sweep** を実行。C69 grep が surface した兄弟サイトを [[ruff-bug-scan-triage]] の三分で triage: **REAL**（生 dict を sort-key に使い null で crash）=`task_queue.get_pending_tasks`(priority+created_at)/`list_tasks`(created_at)・`knowledge` の sort 群・`web _combined_execution_history`(timestamp)／**SKIP**=`task_matcher:45`（match_score は同関数で計算され必ず float＝null 不可・false positive）。**なぜ今これか**: C69 で開いた archetype の兄弟crash（特に `int(t.get("priority",5))` は null で `int(None)` TypeError・**task daemon が 24/7 drain**）を放置しないのが [[get-default-none-footgun]] の「全 call site を着手前に」規律。frontend でなく backend・read-side coerce で最小可逆・回帰検証容易。**落とした候補**: (rrr) DX/trend-watcher・(sss) `id` の別 get-default＝非 sort で theme 外。
  Did    : work/sortkey-null-coerce-sweep-20260620（main 3f3cb68 へ統合）。`core/orchestration/task_queue.py` に module-local helper `_coerce_int`(`is None`+try/except・**priority 0 を保持**)・`_sort_str`(`str(v) if v else ""`) を追加し2 sort サイトを coerce。`core/knowledge/manager.py` に同 helper を追加し **4 sort サイト**（`_load_all_entries`/`get_by_importance`/`get_best_practices`/`get_for_repo`＝usage_count+created_at）を coerce。`web/server.py` の `_combined_execution_history` を `str(... or "")` 化。**初回 grep の盲点を是正**: 単一行 `key=lambda .*\.get(` は **複数行ソートキー**（knowledge の3サイトは `key=lambda record: (` と `.get(` が別行）を取りこぼしていた→`int([a-z_]*\.get(`＋フィールド名の二段 grep で全掃。回帰テスト3本（task_queue=priority/created_at null＋**priority 0 非破壊** `pending[-1]=="c"`／knowledge=usage_count/created_at null の3経路／web=timestamp null）。
  Check  : **test-triage GREEN（backend 1683 passed・既知2失敗=chmod 0o600 のみ・新規回帰0）／ ruff check+format クリーン**。**load-bearing 実証**: 4サイトを旧コードへ一時差し戻し→新規3テストが期待どおり fail（`int(None)` TypeError×task_queue/knowledge・`None < str` TypeError×knowledge/web）後 restore。**敵対的レビュー code-reviewer = APPROVE（blocking 0）**: ① `_coerce_int` は `priority: 0` を default に潰さない（`pending[-1]=="c"` が naive `or 5` と差別化＝load-bearing と実証）、② 非 null 時 sort 順 bit-for-bit 等価（既存 `test_knowledge_get_by_importance_sorted` も pass）、③ **全 sort サイトを grep し完遂を確認**＝他の raw-dict sort-key は `state/manager.py:168/192` のみで既に `str(...)` 防御済・残りは全て **typed dataclass/Pydantic オブジェクト**（`s.started_at`/`p.created_at`/`i.collected_at`）でソート＝`None<str` 経路に到達不能（**型付きは免疫**）、④ 非 sort の `int(x.get())` メトリクス計算は別 archetype で除外妥当。nit2件（helper 重複の DRY・coerce の observability）は **非 blocking で据置**（rule-of-three 未達＋hot sort-key への logging はノイズ）。
  Act    : merged ✅（merge_to_main ゲート通過・--delete-branch、リモート削除エラーは未 push で benign）＝backend correctness sweep。固定化: memory [[get-default-none-footgun]] の sort-key 節に **C70 完遂注記**＝(1) **数値 sort-key に `x or default` は禁物**（`0 or 5==5` で有効値0を静かに破壊＝文字列 `or ""` の逆罠・正は `is None`+try/except）、(2) 初回 grep の複数行 lambda 盲点と二段再 grep、(3) **完遂判定**＝raw dict sort-key のみ対象・typed オブジェクトは免疫・archetype は state/knowledge/task_queue/web で完全閉鎖。**学び: archetype sweep の「完遂」は『全 sort サイト grep → raw-dict（脆弱）vs typed（免疫）に二分 → 既防御を確認』で網を閉じる＝[[ruff-bug-scan-triage]] の三分規律の sort-key 版。**
  Next   : C71 候補（**必ず種別転換**） — (ttt) DX 改善（trend-watcher で Claude Code 最新動向→`.claude` agents/skills/hooks 更新提案を1件取り込む or 評価指標の可視化）、(uuu) 運用層/24h 自律基盤の硬化（デーモン heartbeat/usage-gate の未監査経路）か GUI 機能前進（収益化配線 `_publish_live` 等の承認ゲート付き前進）、(vvv) 別 archetype の backend bug-hunt（非 sort の `int(x.get(k,0))` メトリクス null＝growth_history/understanding_score 等を survey、または core/quality の未監査モジュール）。
           **C69-70 で sort-key footgun archetype は完全掃討＝2連続 correctness。C71 は必ず種別転換（DX・運用層・GUI・別 archetype のいずれか）して多様性回復。3連続 correctness は飽和シグナル。安全＆可逆＆検証可能・nit は defect の時だけを継続。**

Cycle 71 — (DX/safety → correctness ピボット) /evolve 無人ループの安全網強化(guard-bash 破壊的 git ブロック)を**設計完遂したがセンシティブフック承認ゲートで無人適用不可→提案記録**し、C70 が明示的に次送りした knowledge/manager.py の非 sort arithmetic null-crash を出荷  (2026-06-20)
  Plan   : C70 が必須とした**種別転換**を (ttt) DX で実行。trend-watcher で `.claude` 更新提案を取得→#1「破壊的 git コマンドを guard-bash.mjs でブロック」が `/evolve` 24/7 無人ループ自身の安全網強化（auto-commit 未捕捉の新規 untracked module/test を `git reset --hard`/`git clean -fd`/`git checkout -- .`/`git restore`/`git stash drop|clear` が**reflog でも復旧不能**に消す唯一の穴）＝高レバレッジ・既存 `git push --force` ゲートの姉妹・可逆と判断。trend-watcher のバージョン主張は鵜呑みにせず実 guard-bash.mjs を精読し、本当に欠けている保護だけ再設計（#2/#3 は価値低＋#3 自己矛盾＝hallucination で不採用）。
           **しかし guard-bash.mjs 編集がハーネスの sensitive-file 確認ゲートに当たり、無人再開（evolve_resume.ps1・人間不在）では承認不能**。`settings.json` は `Edit(*)` allow なのでこれはプロジェクト deny でなくハーネス組み込みのセキュリティ確認ゲート。**自分自身のセキュリティガードの自律書き換えは正しく人間承認の内側＝自己承認もゲート迂回（settings.json `ask` 経由も「フックが本物のガード」の文書方針の迂回）もしない**（[[main-merge-guard-and-authorization]]）。AskUserQuestion も無人で応答なし。→ guard 強化は**設計・self-test ケース・適用手順まで完成した ready-to-apply 提案**として `docs/plans/proposal-guard-destructive-git.md` に記録。**同じ「24h 自律基盤の堅牢化」テーマ内で**、出荷可能な (vvv) ＝C70 が明示的に次送りした `understanding_score/knowledge` の非 sort `int/float(x.get())` メトリクス null-crash へピボット。受け入れ基準=triage で live-null-able のみに絞る・load-bearing 回帰・全 GREEN・敵対レビュー pass・merged。**落とした候補**: 評価指標可視化（frontend surfacing は C66-68 で飽和）、core/quality bug-hunt（新規 survey コスト）。
  Did    : work/knowledge-metric-null-coerce-20260620（旧名 guard-destructive-git を git branch -m で改名）。[[ruff-bug-scan-triage]] 三分で **REAL のみ**に scope: **`core/knowledge/manager.py` 一本**（C70 が sort サイトを硬化済＝C71 で arithmetic サイトを硬化＝**このファイルの null-coercion 完遂**）。`_coerce_float(value, default)` を `_coerce_int` の対として追加し、①`record_knowledge_access` の `int(record.get("usage_count", 0)) + 1`（live write・知識アクセスのたび発火）②`promote_to_best_practice`/`auto_promote_high_quality` の `float(record.get("quality_score", 0))`（昇格処理を丸ごと落とす）の3サイトを `_coerce_*(record.get(k), default)` へ。`.get(k, 0)` は **キーが null 値で存在すると default でなく None を返す**罠（[[get-default-none-footgun]]）。回帰テスト1本＋happy-path promote(9.0→True)を追加。**SKIP 記録（次 sweep）**: `understanding_score.py`（machine-written・低リスク）/`self_evaluator.py:97`（LLM dict だが try/except で heuristic 劣化＝非クラッシュ）/`growth_history.py`（`**metrics` caller-arg＝disk/LLM free-form でない）/helper 6コピー散在の DRY 統合（6ファイル横断＝別サイクル規模）。
  Check  : ruff クリーン ／ **test-triage GREEN（backend 1685 passed・既知2失敗=chmod のみ・回帰0）**。**load-bearing 実証**: 3サイトを旧コードへ一時差し戻し→新テストが `int(None)` TypeError で fail 後 restore。**敵対的レビュー code-reviewer = APPROVE（blocking 0）**: ① get-default-none 罠の閉鎖を確認（`.get(k,0)` は null 値存在で None 返し→`int/float(None)` crash・`.get(k)`+helper で塞ぐ）、② **非 null で bit-for-bit 等価**（0/0.0・絶対欠落・有効数値文字列いずれも旧と同値・非数値文字列は旧 crash→新 default の robustness 超集合）、③ **網羅性**＝ファイル内全 numeric site（sort 3=C70・arithmetic 3=C71）coerce 済・残 raw なし、④ テスト load-bearing 非 tautological。nit（valid score の happy-path promote も assert）を**採用**（既存に happy-path promote テスト無し→`good`/9.0→True で非 null 経路を固定）。
  Act    : merged ✅（merge_to_main ゲート通過）＝backend robustness（知識昇格/アクセス経路を legacy/手編集/外部の null disk データから守る）＋guard 提案を main に記録（人間レビュー待ち）。固定化（memory [[get-default-none-footgun]] に C71 注記＝arithmetic-position 版）:
           **学び1（archetype の position 横断完遂）**: 同一フィールドの null footgun は **sort-key（比較）／arithmetic（`int()/float()`＋演算）／rendering（payload 描画）の position ごとに別サイト**として現れる。C70 が knowledge の sort を塞いだ後も arithmetic 兄弟が残っていた＝**archetype sweep は『フィールド × position』の二次元で完遂を確認せよ**（ファイル単位 grep で `int(...get())`/`float(...get())` を position 別に洗う）。**学び2（安全境界の尊重を出荷より優先）**: 無人ループが**自分自身のセキュリティガードを書き換える**のはハーネスが正しく人間承認ゲートで止める。budget があっても自己承認/迂回せず、設計を ready-to-apply 提案に落として人間に委ね、**同テーマ内の非ゲート対象へピボットして止まらない**のが [[main-merge-guard-and-authorization]] の実践（正直さ＝ゲートに当たった事実をそのままログ）。
  Next   : C72 候補 — (www) **guard-destructive-git 提案を人間承認後に適用**（次回 manned セッション or ユーザー承認時＝proposal-guard-destructive-git.md の手順）、(xxx) **メトリクス null-crash sweep の continuation**＝C71 が SKIP 記録した `understanding_score.py`(record_exploration の exploration_count++)＋`self_evaluator.py`(LLM score・try/except 劣化を観測化)＋他の `int/float(x.get())` arithmetic を position 別に横ぐし、(yyy) **多様性転換を強制**＝DX(評価指標可視化)/運用層(daemon heartbeat 未監査)/GUI のいずれか。
           **C69-71 が実質 correctness 3連（C71 は guard ピボットだが結果は correctness）＝C72 は必ず非 correctness（DX/運用層/GUI）へ。guard 提案は manned 時に最優先で適用。helper 6コピーの DRY 統合は別途 1サイクルの価値。安全＆可逆＆検証可能・nit は defect/strengthening の時のみを継続。**

Cycle 72 — (運用層/Windows 堅牢性) コンソール窓の flash を消す windowless-subprocess sweep を完遂＝detached daemon は pythonw.exe で起動・対話経路の git は CREATE_NO_WINDOW・初回が見落とした web/server.py の git 重複コピーを補完し pythonw 分岐を Linux CI でも実行  (2026-06-20)
  Plan   : 中断していた C72（branch `work/nowindow-git-subprocess`）を**自動再開**で完遂。既コミット済の実装（daemon_registry を `_windowless_python()` で pythonw 化＋abstract_goal_pipeline/repo_resolver の git に `no_window_kwargs()`）を**未テスト・未レビュー・未マージ・未ログ**の状態から検証→出荷。**なぜ今これか**: 中断点の再開が最優先＝既存変更を闇に葬らず正しくゲートを通す。**resume 時の追加調査で sweep の網羅性を再検証**し、全 subprocess サイト（detector/lint_checker/safe_executor/claude_code/headless_driver は既適用・daemon は pythonw）を grep して **web/server.py:3762 の `_git_remote_github_repo`（repo_resolver の near-duplicate）だけ未適用**を発見＝初回 sweep の漏れ。受け入れ基準=漏れ補完＋pythonw 分岐の直接テスト＋全 GREEN＋敵対レビュー pass＋merged。
  Did    : work/nowindow-git-subprocess-20260620（main 1162f6f へ統合）。① `web/server.py` の git remote get-url に `no_window_kwargs()`（関数内ローカル import＝同ファイルの core.runtime 遅延 import パターン踏襲）を適用し sweep 完遂。② `tests/test_daemon_registry.py` に `_windowless_python()` の**ポータブル直接テスト3本**（os_name 注入で nt+兄弟 pythonw 存在→差し替え／欠落→フォールバック／非 nt→no-op を tmp_path の偽 exe で実行）。既存の `build_command == [registry._windowless_python(), ...]` 断言は**構造 pin だけで pythonw 分岐を Linux CI で一切実行しない（dark branch）**＝レビュー指摘の coverage 穴を直接テストで閉鎖。
  Check  : ruff check+format クリーン ／ **test-triage GREEN（backend 1685 passed・4 skip=chmod 0o600×2＋symlink＋POSIX zombie・新規回帰0）**。**敵対的レビュー code-reviewer = APPROVE-WITH-NITS（blocking 0）**: ① `_windowless_python` は frozen 早期 return で不変・`candidate.exists()` でフォールバック・非 nt skip・pythonw 自己指定も冪等、② **pythonw でも `Popen(stdout=log_handle)` の明示ハンドルで daemon の print はログへ届く**（コンソール窓割り当てと stdio ハンドル継承は独立・watchdog が既に pythonw+ログで先例）＝liveness/pid/detach 不変、③ CREATE_NO_WINDOW は console-visibility flag で pipe capture と直交（`capture_output`/`text` を壊さない・nt のみ）、④ web/server.py の 404 不変。nit を採用: **build_command 断言は自分自身比較で pythonw 分岐が dark→ポータブル直接テスト追加**（採用済）／未コミット web/server.py を merge 前に commit（採用＝明示コミット）／`_git_remote_github_repo` の重複は scope 外で follow-up（据置）。
  Act    : merged ✅（merge_to_main ゲート通過・--delete-branch、ローカル削除は未 push remote で benign skip）＝運用層/Windows 堅牢性（GUI/ヘッドレス運用で git・daemon 起動のたびに空コンソール窓が flash しフォーカスを奪う UX 実害を一掃）。固定化: memory [[windows-process-portability]] に **windowless sweep（C72）** を追記＝コンソール窓は2系統で消す（①対話経路の短命 subprocess は `no_window_kwargs()`＝CREATE_NO_WINDOW／②detached daemon は CREATE_NO_WINDOW が DETACHED_PROCESS と排他なので **GUI-subsystem の pythonw.exe** で起動）、sweep 完遂は全 subprocess サイト grep＋web/server.py の git 重複コピー込みで一掃、`_windowless_python` の pythonw 分岐は os_name 注入のポータブルテストで Linux CI でも実行（構造 pin だけでは dark）。
  Next   : C73 候補（**多様性転換**） — (zzz) **coerce helper の DRY 統合**＝C70/C71 が2度「別サイクルの価値」と明示した `_coerce_int`/`_coerce_float`/`_sort_str` の3ファイル重複（knowledge/task_queue はバイト同一・handoff_optimizer は1引数版）を共有モジュールへ集約（保守性/DX・既存回帰でカバー）、(xxx) メトリクス null-crash sweep continuation（understanding_score/self_evaluator）、(yyy) 運用層/GUI 前進。
           **C72 は Windows 堅牢性（correctness 隣接）＝C73 は DX/保守性（coerce DRY 統合）へ転換して多様性回復。安全＆可逆＆検証可能・nit は defect/strengthening の時のみを継続。**

Cycle 73 — (DX/保守性) 生 JSON 防御 coerce helper の3ファイル重複を `core/persistence.py` の公開関数へ集約＝C70/C71 が2度「別サイクルの価値」と明示した DRY 統合を完遂し correctness 連続から保守性へ多様性転換  (2026-06-20)
  Plan   : C72 が予告した (zzz) **coerce helper DRY 統合**を実行。`_coerce_int`/`_coerce_float`/`_sort_str` が **knowledge/manager・orchestration/task_queue・hierarchy/handoff_optimizer の3ファイルに重複**（knowledge/task_queue の `_coerce_int`/`_sort_str` はバイト同一・handoff_optimizer の `_coerce_float` は1引数で 0.0 固定）。統合先は **`core/persistence.py`**＝write-side の原子性（`atomic_write_text`）と対になる **read-side の生 JSON 防御パース**で同テーマ・リーフモジュール（os/tempfile/pathlib のみ＝循環参照リスク無し）。**なぜ今これか**: C69-72 が実質 correctness 連続＝[[get-default-none-footgun]] の正準形をコピペで散在させたまま放置せず single source 化するのは複利的（次に同 archetype を直す時に1箇所で済む）・既存回帰（knowledge/task_queue/handoff）でカバー済＝低リスク・多様性回復（correctness→保守性/DX）。受け入れ基準=3ファイルの重複除去＋公開 helper の load-bearing 直接テスト＋全 GREEN＋敵対レビューで behavior-equivalence 確認＋merged。**落とした候補**: (xxx) メトリクス null sweep＝correctness 続行で多様性に反する、(yyy) GUI＝C66-68 で飽和。
  Did    : work/coerce-helpers-dry-20260620（main へ統合）。`core/persistence.py` に公開 `coerce_int(value, default=0)`/`coerce_float(value, default=0.0)`/`coerce_sort_str(value)` を追加（priority 0 を破壊しない `is None`＋try/except・`value or default` 罠を docstring で明示）。3 consumer から重複ローカル定義を削除し `from core.persistence import ...` へ（task_queue は既存 `atomic_write_text` import を拡張）、全 call site を `_coerce_*`→`coerce_*` に rename。`tests/test_persistence.py` に直接ユニットテスト5本（**0 保持** `coerce_int(0,5)==0`＝`value or default` 回帰を捕捉する load-bearing／None・非数値→default／非 str 文字列化／falsy→空文字）。
  Check  : ruff check+format クリーン ／ **test-triage GREEN（backend 1693 passed・4 skip=chmod×2＋symlink＋POSIX zombie・新規回帰0）**。**敵対的レビュー code-reviewer = APPROVE（blocking 0）**: ① handoff の1引数→default 0.0 は `coerce_float(raw.get("reach"))` で 0.0 を再現＝byte-equivalent（旧は1引数のみ呼出＝別 default 依存サイト無し）、② task_queue priority の **0 保持不変**を `coerce_int(0,5)==0` で実証（`value or default` でない）、③ persistence はリーフで**循環参照無し**（4モジュール同時 import 成功）、④ 削除した private 名の dangling 参照ゼロ（repo grep）・`Any` は3モジュールで依然使用（orphan import 無し）、⑤ 新テストは `int(value or default)` 回帰を実際に fail させる非 tautological、⑥ default 引数（0/0.0）は全既存明示渡しと一致＝missing-arg を隠さない。nit（docstring の call-site 固有文脈が暗黙化・未コミット）は採用（コミット済）／framing は acceptable。
  Act    : merged ✅（merge_to_main ゲート通過）＝DX/保守性（[[get-default-none-footgun]] の正準 coerce を single source 化＝二度とコピペで腐らない）。固定化: memory [[get-default-none-footgun]] へ **coerce helper は `core/persistence.py` に集約済**を追記＝今後 raw JSON の null/非数値防御は新規ローカル定義でなく `coerce_int/coerce_float/coerce_sort_str` を import せよ（write-side `atomic_write_text` と対）。**学び: 確立 archetype の正準実装が3+ファイルにコピペ散在したら『別サイクルの価値』として DRY 統合を1本起こす＝rule-of-three 到達後の single source 化は次の同型修正を1箇所化する複利投資。統合先は機能テーマで選ぶ（read-side 防御は write-side 原子性と同じ persistence.py）。**
  Next   : C74 候補 — (aaa) **メトリクス null-crash sweep continuation**＝C71 が SKIP 記録した `understanding_score.py`(record_exploration の exploration_count++)＋`self_evaluator.py`(LLM score の try/except 劣化を観測化) を新 `coerce_*` で position 別に横ぐし、(bbb) **運用層/24h 自律基盤の硬化**（daemon heartbeat/usage-gate の未監査経路）、(ccc) **DX/Claude Code ベストプラクティス**（trend-watcher で `.claude` 更新提案を1件・ただし sensitive-hook ゲートに当たらないもの）。
           **C73 で DX/保守性へ転換完了。C74 は (aaa) で新 coerce helper を早速使う continuation（複利の実演）か、(bbb)(ccc) で運用層/DX へさらに多様化。guard-destructive-git 提案は manned 時に最優先適用。安全＆可逆＆検証可能・nit は defect/strengthening の時のみを継続。**
