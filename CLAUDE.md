...
> Claude Code / Claude を含むすべての外部AIコーディングエージェントは、まず `AGENTS.md` を
> 読んでから作業してください。本ファイルは Claude Code 用の入口で、指示を二重管理しないために
...
リポジトリ直下の `skills/` `agents/` `commands/` は **RepoCorp の実行時概念**（YAMLで定義する
内部エージェント/スキル、CLIサブコマンド）であり、Claude Code の `.claude/skills` 等とは
**無関係**です。詳細は AGENTS.md と `docs/architecture.md` の「エージェントの2つの平面」を参照。
