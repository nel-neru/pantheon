...
// 旧・既知失敗（Windows chmod）の防御的フォールバック。これらは 2026-06-20 以降
// `skipif sys.platform=="win32"` で Windows ではスキップされ FAILED に出ない（＝通常は
// このセットにヒットしない）。skipif が将来外れて再び落ちた場合に回帰と誤判定しないための
// 保険として残す。**ファイル(::クラス)::関数 のフル nodeid** で照合する。
