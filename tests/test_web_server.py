...
def test_organization_structure_has_no_fabricated_dependencies(tmp_path, monkeypatch):
    """組織構造に依存関係を捏造しないこと（回帰）。

    Team モデルにチーム間依存の概念は存在しないため、全チームの depends_on は None であるべき。
    以前は ``_serialize_org_structure`` が ``previous_team_name or previous_division_name``
    （＝列挙順の直前チーム名）を代入し、UI(OrgsPage の DependencyList)が実在しない依存グラフを
    「定義された依存関係」として描画していた（engineering-integrity の fake-data 違反）。
    """
    psm = server.PlatformStateManager(platform_home=tmp_path)
    monkeypatch.setattr(server, "_psm", lambda: psm)

    # 複数 Division/Team を持つ実 org を生成（旧バグは 2 番目以降のチームで踏むため複数必須）。
    install = client.post("/api/company-plugins/note_sales/install", json={})
    assert install.status_code == 200, install.text
    org = psm.load_organization_by_name(install.json()["org_name"])
    assert org is not None

    structure = server._serialize_org_structure(org)
    teams = [team for division in structure for team in division["teams"]]
    assert len(teams) >= 2, "複数チームの org でないと回帰を検出できない"
    fabricated = [(t["name"], t["depends_on"]) for t in teams if t["depends_on"] is not None]
    assert fabricated == [], f"依存関係が捏造されている: {fabricated}"
