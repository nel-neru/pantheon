...
def test_pickup_tolerates_null_expected_impact(tmp_path):
    """expected_impact が null の生 JSON 提案（legacy/手編集/外部生成）でソートが落ちない。

    回帰: ``key=lambda p: (..., p.get("expected_impact", ""))`` は **キーが null 値で
    存在すると "" でなく None を返す**ため、``None < str`` のソート TypeError が
    pickup_and_prioritize_proposals 全体を落としていた（scheduler の try/except に飲まれ、
    自己改善ループが静かに永久停止）。修正後は coerce されて 2 件を順序付けして返す。
    """
    org = create_default_organization("NullImpactOrg", "self improve")
    sm = RepoStateManager(tmp_path, "NullImpactOrg")

    improvements_dir = sm.state_dir / "improvements"
    improvements_dir.mkdir(parents=True, exist_ok=True)
    # 2 件とも同一 priority（ソート第1キーが等しい）→ 第2キー（expected_impact）が比較され、
    # 片方が null だと旧コードは TypeError。両ファイルを ImprovementProposal を経由せず直接書く。
    (improvements_dir / f"{uuid4()}.json").write_text(
        json.dumps(
            {
                "id": str(uuid4()),
                "title": "null-impact proposal",
                "priority": "low",
                "expected_impact": None,  # ← legacy/外部 JSON の null
                "status": "pending",
    ...
    # 旧コードはここで TypeError を送出していた。
    prioritized = asyncio.run(loop.pickup_and_prioritize_proposals())

    assert len(prioritized) == 2
    titles = {p["title"] for p in prioritized}
    assert titles == {"null-impact proposal", "string-impact proposal"}
