...
def main() -> None:
    ...
    # vault_sync daemon（Obsidian 互換 Vault の自動同期）の frozen 自己再起動エントリ。
    if argv and argv[0] == "--vault-sync-daemon-run":
        from core import _vault_sync_daemon_runner

        sys.argv = [sys.argv[0], *argv[1:]]
        _vault_sync_daemon_runner.main()
        return
