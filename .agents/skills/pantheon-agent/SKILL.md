---
name: pantheon-agent
description: How to correctly add or modify a Pantheon in-app agent and/or skill (the product's OWN agent framework — not Codex subagents). Use when editing agents/, the AgentSkill enum, or skills/*.yaml, or when asked to add a specialist agent or skill.
paths:
  - "agents/**/*.py"
...
> NOT Codex subagents in `.Codex/agents/`. They are different layers.
...
2. Generation goes through the `Codex` CLI provider (`core/runtime/Codex`); guard offline
