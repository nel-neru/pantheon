...
class PreTaskOrchestrator:
    ...
    def analyze(
        ...
        """
        profile = TASK_ORCHESTRATION_PROFILES.get(
            task_type, TASK_ORCHESTRATION_PROFILES["default"]
        )
        ...
        logger.info(
            ...
            task_type, analysis.recommended_pattern, analysis.recommended_agent_ids,
        )
        return analysis
    ...
    def _select_agents(
        self, required_skills: List[str], min_agents: int = 1
    ) -> List[str]:
        """
    ...
    async def execute(
        ...
        if pattern == OrchestrationPattern.SINGLE_AGENT:
            return await self._execute_single(task, analysis, agent_factory)
        elif pattern == OrchestrationPattern.SEQUENTIAL_PIPELINE:
            return await self._execute_sequential(task, analysis, agent_factory)
        elif pattern == OrchestrationPattern.REVIEW_LOOP:
            return await self._execute_review_loop(task, analysis, agent_factory)
        elif pattern == OrchestrationPattern.PARALLEL_THEN_MERGE:
            return await self._execute_parallel(task, analysis, agent_factory)
        else:
            return await self._execute_single(task, analysis, agent_factory)
    ...
    async def _execute_single(self, task, analysis, agent_factory):
        ...
        if agent is None:
            from agents.base import AgentResult
            return AgentResult(success=False, error=f"Agent not found: {analysis.recommended_agent_ids[0]}")
        result = await agent.run(task)
        self._record_execution(task, analysis, result)
        return result
    ...
    async def _execute_sequential(self, task, analysis, agent_factory):
        ...
        self._record_execution(task, analysis, last_result)
        return last_result

    async def _execute_review_loop(self, task, analysis, agent_factory):
        """実行 → レビュー → (品質不足なら再実行) ループ。"""
        main_agent_id = analysis.recommended_agent_ids[0] if analysis.recommended_agent_ids else None
    ...
    async def _execute_parallel(self, task, analysis, agent_factory):
        ...
        best = successful[0]
        self._record_execution(task, analysis, best)
        return best
    ...
    def _record_execution(self, task, analysis: TaskAnalysis, result) -> None:
        """実行結果をログに記録し、パターン検出器・パターンストアに通知する。"""
        ...
        # OrchestrationPatternStore に記録（パターン学習）
        if self._pattern_store:
            from core.orchestration.orchestration_pattern_store import PatternRecord
            try:
                self._pattern_store.record(PatternRecord(
                    task_type=analysis.task_type,
                    pattern=analysis.recommended_pattern,
                    agent_ids=analysis.recommended_agent_ids,
                    success=success,
