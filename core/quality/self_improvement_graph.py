...
async def execute_improvement(state: SelfImprovementState) -> Dict[str, Any]:
    ...
    # 改善実行担当はスキルマッチ（CapabilityRegistry 経由 AgentFactory）で選ぶ。
    # 解決できない場合のみ従来どおり先頭エージェントにフォールバック。
    executor_agent = None
    try:
        from agents.agent_factory import AgentFactory

        executor_agent = AgentFactory().create("agent:improvement_executor")
    except Exception:  # noqa: BLE001 - 解決不能時はフォールバック
        executor_agent = None
...
def build_self_improvement_graph(
    ...
    ``checkpointer_path`` を省略すると、チェックポイント DB はグローバル状態の
    ルート（``~/.pantheon``）配下に作成する。以前は cwd 相対の既定値だったため、
    実行ディレクトリ次第で DB が散乱し（非交渉事項「状態は ~/.pantheon」違反）、
    リポジトリを汚していた。明示パスを渡せばそのまま使う（テスト等）。
    """
    ...
    # SQLite による永続化チェックポイント。既定はグローバル状態ルート配下。
    if checkpointer_path is None:
        db_path = get_platform_home() / "self_improvement_checkpoints.db"
        db_path.parent.mkdir(parents=True, exist_ok=True)
        checkpointer_path = str(db_path)
    elif checkpointer_path != ":memory:":
        # 明示パスでも親ディレクトリが無いと sqlite3.connect が失敗するため担保。
        # （":memory:" は実ファイルを持たない sqlite 特殊指定なので mkdir しない）
...
async def run_improvement_for_organization(
    ...
    # チェックポイント用 sqlite 接続は graph に紐付く。実行後に必ず閉じてリークを防ぐ
    # （長時間常駐のループで graph を作り続けると未クローズ接続が溜まる）。DB ファイル自体は
    # 残るため、後続の resume は新しい接続で同じ DB を開き直せる。
