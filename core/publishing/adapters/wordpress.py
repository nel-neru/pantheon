...
**接続フローは Phase 2（現状 end-to-end 未開通）**: ログイン URL がサイト URL 依存のため
``connect.py`` の ``LOGIN_URLS`` / CLI の ``CONNECTABLE_PLATFORMS`` は wordpress を含まない
（CLI は ``interactive_login`` が「接続フロー未対応」を返し、Web の login API は ``LOGIN_URLS`` 非掲載で
``unsupported`` を返す＝両ゲートとも ``LOGIN_URLS`` 由来）。よって本アダプタの実投稿経路はセッションが
用意できる Phase 2 まで到達しない（テストは fake launcher と storage_state を直接注入して検証）。
    ...
    async def _publish_live(self, content: PublishContent, target: PublishTarget) -> PublishResult:
        ...
        if not store.is_connected(self.platform):
            return PublishResult(
                ...
                mode=target.mode,
                error=(
                    "WordPress は接続フロー未対応です（サイト URL 依存ログインは Phase 2）。"
                    "現状 assisted で接続できるのは note / X です"
                ),
            )
        ...
        try:
            ...
            try:
                ...
                pass
            hint = (
                "（保存済みセッションが無効かもしれません。WordPress の接続フローは Phase 2 です）"
            )
            return PublishResult(
                ok=False,
                platform=self.platform,
