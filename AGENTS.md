# RepoCorp AI — Agent Instructions

このファイルはAIエージェント（GitHub Copilot, Claude等）がこのリポジトリで作業する際に
最初に読むべきコンテキストファイルです。

> **作業を引き継ぐ / 別デバイスで再開する場合は [`docs/HANDOFF.md`](docs/HANDOFF.md) を読んでください。**
> 進行中の改善ループ（Phase 5: 100件）の現在地・再開点・検証コマンドを集約しています。
> 生きたチェックリストは [`docs/improvement_backlog.md`](docs/improvement_backlog.md)。

## プロジェクト概要

RepoCorp AIは「個人開発者が自分専用の自己成長型AI組織を立ち上げ、共に進化させる」プラットフォームです。
CLI / Web UI / 自律実行パイプラインを通じて、Organizationの作成、コード分析、改善提案、承認、自己改善を支援します。

## エージェントの2つの平面（必読・混同しないこと）

- **平面A（ビルド時 / 外部）**: あなた（Claude Code / Codex / Cursor / Copilot / Gemini 等の外部AIツール）が
  RepoCorp 自身を開発する層。**正典は本 `AGENTS.md`**。`CLAUDE.md` / `GEMINI.md` /
  `.github/copilot-instructions.md` / `.cursor/rules/repocorp.mdc` は本ファイルへのリダイレクトに過ぎない。
- **平面B（実行時 / 内部）**: RepoCorp 自身が「ユーザーの契約しているLLM」で動く層。
  `skills/*.yaml` + `agents/definitions/*.yaml` + `core/llm`（プロバイダー抽象）で構成。
- リポジトリ直下の `skills/` `agents/` `commands/` は **RepoCorp の実行時概念**であり、
  Claude Code の `.claude/skills`・`.claude/agents`・`.claude/commands` とは**無関係**（名前が似ているだけ）。
- 詳細は [`docs/architecture.md`](docs/architecture.md) の「エージェントの2つの平面」「LLM アクセス」。

## ディレクトリ構造

- `main.py` — `repocorp` CLI エントリーポイント。全サブコマンドを定義
...
- `core/` — 中核ロジック
  ...
  - `platform/` — グローバル状態 (`~/.repocorp`) 管理
  - `state/` — 各リポジトリ内 `.repocorp/` 状態管理
  ...
  - `llm/` — プロバイダー非依存のLLM抽象（anthropic/openai/groq/github_models/gemini）。
    `client.py`(同期ブリッジ+既定クライアント解決) / `tool_schema.py`(tool中立化) /
    `json_extract.py`(堅牢JSON抽出) / `capabilities.py`(能力レジストリ) / `model_registry.py`(モデル集約)
...
- `core/state/manager.py` — 各対象リポジトリ内 `.repocorp/` の状態・提案・決定管理
...
- **LLM 呼び出しは必ず `core/llm` 経由（プロバイダー非依存）**。既定クライアントは
  `core.llm.get_default_llm_client()`、構造化(JSON)出力は `generate_json` を使い、特定プロバイダーSDKに直結しない
- SpecialistAgent の `skills` は最低2個、最大3個
- 状態はグローバルなら `~/.repocorp`、対象リポジトリ固有なら `<repo>/.repocorp` に保存する
...
- `core/state/manager.py` — 各リポジトリ内の `.repocorp` 永続化
