...
- **エンジニアリング誠実性（NON-NEGOTIABLE・その場しのぎ禁止／見かけUI禁止）**: 失敗（テスト・
  ビルド・実行時エラー・フレーク）は**症状を黙らせず根本原因を実コードで突き止めて直す**。
  テスト/フック/ゲートのバイパス（`--no-test`・`--no-verify`・閾値いじり）、`skip`/`xfail`/
  アサート削除/`try/except: pass` での握り潰し、「とりあえず動く」コミットは禁止（`skipif` の正当な
  用途は文書化された真のプラットフォーム非互換のみ。フレークは skip でなく根治する）。UI は実際に
  機能する経路（component→API→backend→state）を持つこと — 未配線ボタン・未接続UIを実装済みとして
  出す・ダミー/モックデータで「動いて見せる」・コピーが約束した値を描画しない、は禁止。未完の部分は
  隠さず正直にラベル（「未接続」「Phase 2」「概算/予測」「確定データなし」）。根治できないなら
  その場しのぎを入れず「未完/既知問題」として正直に報告しユーザーに委ねる。
  詳細: `.claude/rules/engineering-integrity.md`（`revenue-integrity.md` の一般化）。
